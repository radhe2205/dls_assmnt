...
def get_negative_pair_samples(signal, pair_per_speaker = 45): # Signal 500 x T x 513
    ...
    for cur_speaker in range(num_speakers):
        first_speaker_samples = [i % sample_per_speakers for i in range(pair_per_speaker)]
...
class SpeakerIdenModel(nn.Module):
    def __init__(self):
        ...
        self.rnn_layers = nn.GRU(input_size=513, num_layers=3, hidden_size=128, batch_first=True)
    ...
    def forward(self, samples): # batch, seq * 2, 513
        ...
        vec = (o1n * o2n).sum(-1).unsqueeze(-1)

        return self.last_layer(vec).squeeze(-1)

        # o1n = self.batch_norm(o1n[:,-1,:])
        # o2n = self.batch_norm(o2n[:,-1,:])

        # return self.sigmoid((o1n * o2n).sum(dim = -1))

        # o1n = self.conv_layers(samples[:, 0, ...].reshape(-1, 1, 513)).reshape(samples.shape[0], samples.shape[2], -1)
        # o2n = self.conv_layers(samples[:, 1, ...].reshape(-1, 1, 513)).reshape(samples.shape[0], samples.shape[2], -1)
        #
        # o1n, (h1n, c1n) = self.rnn_layers(o1n)
        # o2n, (h2n, c2n) = self.rnn_layers(o2n)
        #
        # emb1 = self.fc_layers(o1n[:, -1, :])
        # emb2 = self.fc_layers(o2n[:, -1, :])
        #
        # return self.sigmoid((emb1 * emb2).sum(dim = 1)) # batch, 32, 64
...
def train_speaker_model(train_options):
    ...
    max_acc, temp = get_val_accuracy(model, train_loader)
